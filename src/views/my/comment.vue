      ...
      <van-tab :title="val.title" v-for="val of tabs" :key="val.id">
        ...
        <Scroll class="scroll" v-if="currentActive == 1">
          <div class="warp">
            ...
            <div
              class="item border-bottom"
              v-for="(val, index) in dataArr"
              :key="index"
            >
              <img
                :onerror="defaultImg"
                :src="url + val.goodList[0].goods_img"
                alt=""
                srcset=""
              />
              <div>
                <p>{{ val.goodList[0].title }}</p>
...
.evaluate-warp .scroll .warp .item div .span2 {
  color: #333;
  border: 1px solid #666;
}
.evaluate-warp .scroll .warp .item div span {
  align-self: flex-end;
...
.evaluate-warp .scroll .warp .item div span i {
  vertical-align: -1px;
  margin-right: 5px;
}
</style>
